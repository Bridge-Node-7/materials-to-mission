...
def test_visual_foundation_docs_preserve_key_distinctions() -> None:
    register = (ROOT / "docs/PROGRAM_REGISTER.md").read_text(encoding="utf-8")
    assert "unknown is not contradicted" in register
    assert "claim support is not pathway assessment" in register
    assert "evidence-supported action is not human decision" in register
    assert "No composite readiness score" in register
