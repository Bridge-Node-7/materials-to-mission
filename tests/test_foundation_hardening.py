...
def test_v071_durable_source_identity_is_explicit() -> None:
    ...
    assert facts["version"] == facts["source_version"] == "0.7.1"
    assert facts["immediate_prior_immutable_release"] == {
        "tag": "v0.7.0",
        "verified_signed_tag_target": "8210af0362b32baf3f4e30c2ccb5ca7d8643b838",
...
def test_release_bound_source_does_not_self_declare_mutable_github_state() -> None:
    active_source = "\n".join(
        ...
        for relative in ("docs/CURRENT_STATE.md", "PROJECT_FACTS.json", "RELEASE_NOTES_v0.7.1.md")
    )
    for volatile in (
...
def test_release_title_is_derived_from_version_matched_notes_h1() -> None:
    ...
    notes_h1 = (ROOT / "RELEASE_NOTES_v0.7.1.md").read_text(encoding="utf-8").splitlines()[0]
    assert notes_h1 == "# Materials-to-Mission v0.7.1 — Foundation Hardening"
